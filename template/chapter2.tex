...
The work presented in this thesis is based on two main areas of research: the evaluation of LLMs(LLMs) and Information Retrieval(IR).
...
\\
With the release of GPT-3~(\cite{brown:2020}), the size of datasets used to train LLMs, as well as the number of parameters in the models increased significantly.
While GPT-2 in its largest version has a total of 1.5 billion parameters, GPT-3 has 175 billion parameters.
As \cite{wei:2022} show, this scale not only leads to improvements over previously used benchmark compared to smaller models, but also to what they call \textit{emergent abilities}.
...
None of those capabilities are present in smaller models, with e.g. the largest version of GPT-2 performing poorly on translation and summarization, as shown in the original paper~(\cite{radford:2018}).
\\
...
% For early models like BERT and the first version of GPT, extractive QA is the most prominent form of QA tasks.
% Here, the task is to answer questions based on a given context, by selecting a span of text from the context, e.g. using the SQuAD dataset by \cite{rajpurkar:2016}.
% The next versions of models are additionally benchmarked on answering single choice questions, where the model has to select the correct answer from a set of possible answers.
% This can be done either with context, or without.
% Additionally, models are evaluated on questions where the answers are single numbers, entities or other versions of "fill in the blank" questions.
\\
...
For each token in the provided context, the model assigns a probability of the token being the starting or ending token of the answer span~(\cite{devlin:2018}).
\\
Later models like GPT-3 are able to directly generate the answer from the context and the questions without any fine-tuning, by using the zero-shot, single-shot or multi-shot capabilities of the model~(\cite{brown:2020}).
...
While most datasets are single choice datasets, some datasets (like MultiRC by \cite{khashabi:2018}) are multiple choice, so that the model has to check each answer for correctness, instead of just selecting the best fitting one.
Questions for single and multiple choice QA often stem from official exams, like the MMLU dataset~(\cite{hendrycks:2020}), which combines question from many exams like the United States Medical Licensing Examination or the Examination for Professional Practice in Psychology.
...
This dataset was used to by~\cite{nakano:2021} to fine-tune GPT-3 for the task of long form QA without using the context documents.
...
\\
None of the papers for current, main-stream LLMs like GPT-3~(\cite{brown:2020}), GPT-4~(\cite{openai:2023}) or Llama 2~(\cite{touvron:2023}) include evaluations on common benchmarks for this category.
...
\end{itemize}
In addition to these metrics, \cite{xu:2023} evaluate two different versions of fine-tuned metrics.
...
\\\\
\cite{krishna:2021} investigates in more detail the problems of reference based evaluation metrics like ROUGE-L. 
...
Based on the findings by \cite{xu:2023} how shows that of the automated metrics the transformer-based models perform best, we compare multiple transformer-based model for the retrieval step.
\\
...
A brief overview of a retrieval pipeline based on ``An introduction to information retrieval'' by \cite{manning:2009} is provided now.
\\
...
Divergence from Randomness (DFR) is a framework in IR that assigns term weights based on the divergence of the actual within-document term frequency distribution from a random term frequency distribution~(\cite{amati:2006}).
...
Due to their fine-grained interaction modeling, cross-encoders outperform bi-encoders in terms of precision in retrieving relevant documents as shown by \cite{thakur:2020} and \cite{rosa:2022}.
...
% Arrows
\draw[-{Latex[length=3mm]}] (query) -- (q1);
\draw[-{Latex[length=3mm]}] (query) -- (q2);
\draw[-{Latex[length=3mm]}] (query) -- (q3);
\draw[-{Latex[length=3mm]}] (q1) -- (qencoder);
\draw[-{Latex[length=3mm]}] (q2) -- (qencoder);
\draw[-{Latex[length=3mm]}] (q3) -- (qencoder);
\draw[-{Latex[length=3mm]}] (qencoder) -- (qe1);
\draw[-{Latex[length=3mm]}] (qencoder) -- (qe2);
\draw[-{Latex[length=3mm]}] (qencoder) -- (qe3);
\draw[-{Latex[length=3mm]}] (doc) -- (d1);
\draw[-{Latex[length=3mm]}] (doc) -- (d2);
\draw[-{Latex[length=3mm]}] (doc) -- (d3);
\draw[-{Latex[length=3mm]}] (d1) -- (docencoder);
\draw[-{Latex[length=3mm]}] (d2) -- (docencoder);
\draw[-{Latex[length=3mm]}] (d3) -- (docencoder);
\draw[-{Latex[length=3mm]}] (docencoder) -- (de1);
\draw[-{Latex[length=3mm]}] (docencoder) -- (de2);
\draw[-{Latex[length=3mm]}] (docencoder) -- (de3);
\draw[-{Latex[length=3mm]}] (qe1.south) -- (maxSim1);
\draw[-{Latex[length=3mm]}] (qe2.south) -- (maxSim2);
\draw[-{Latex[length=3mm]}] (qe3.south) -- (maxSim3);
\draw[-{Latex[length=3mm]}] (de1.south) -- (maxSim1);
\draw[-{Latex[length=3mm]}] (de1.south) -- (maxSim2);
\draw[-{Latex[length=3mm]}] (de1.south) -- (maxSim3);
\draw[-{Latex[length=3mm]}] (de2.south) -- (maxSim1);
\draw[-{Latex[length=3mm]}] (de2.south) -- (maxSim1);
\draw[-{Latex[length=3mm]}] (de2.south) -- (maxSim2);
\draw[-{Latex[length=3mm]}] (de3.south) -- (maxSim3);
\draw[-{Latex[length=3mm]}] (de3.south) -- (maxSim2);
\draw[-{Latex[length=3mm]}] (de3.south) -- (maxSim3);
...
\end{tikzpicture}
\caption{ColBERT Retrieval Model. The query and document are processed separately to generate token embeddings. The maximum cosine similarity between all token embeddings of the query and document is calculated and summed over all query tokens, the sum of which is the relevance score.}
\label{fig:colbert}
\end{figure}
...
\subsubsection{ColBERT V2}\label{sec:colbert-v2}
