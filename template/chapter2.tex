...
The rise of Large Language Models (LLMs), started with the introduction of the new Transformer architecture~(\cite{vaswani_attention_2023}), followed by the release of models like BERT~(\cite{devlin:2018}) and GPT-2~(\cite{radford:2018}).
...
\subsection{Transformer-based Retrieval Models}\label{transformer-retrieval-models}
...
\subsubsection{Cross-Encoders vs. Bi-Encoders}\label{cross-encoders}
\begin{table}[h]
    \centering
...
\subsubsection{MonoT5 + DuoT5}\label{monot5-duot5}
MonoT5 and DuoT5~(\cite{roberts:2019}), are based on previous work by \cite{nogueira:2019}, who introduced monoBERT and duoBERT, first applying transformer module to the task of document ranking.
...
MonoT5 and DuoT5 both belong to the family of cross-encoder models, as described in Section~\ref{cross-encoders}, sharing the general characteristics of those models.

\subsubsection{ColBERT}\label{colbert}
...
\section{Evaluation of NLP tasks using Information Retrieval}\label{eval-nlp-ir}