...
ColBERTv2~\citep{santhanam:2021} is directly based on the late-interaction architecture of ColBERT, but adds improvements to the architecture and the training process.
\\
...
\cite{duh:2008} argues that, considering the final goal is to compare different translation systems, a direct comparison between the systems is preferable to evaluating their quality individually.
He shows that ranking methods like RankSVM~(\cite{joachims:2002}) and RankBoost~(\cite{freund:2003}) can be applied to rank different candidate translations against the reference, generating similar scores to BLEU on the same feature set.
...
\\
Another approach based on learning-to-rank methods is proposed by \cite{li:2013}.
...
Similar to \cite{duh:2008}, they show that the ranking based methods correlate stronger with human judgement, compared to BLEU-like metrics.
\\
